...
int perturb_indices_of_perturbs(
  ...
  ppt->has_source_delta_drad = _FALSE_;
  ...
  ppt->has_source_theta_drad = _FALSE_;
  ...
  for (index_md = 0; index_md < ppt->md_size; index_md++) {
    ...
    if (_scalars_) {
      ...
      if (ppt->has_density_transfers == _TRUE_) {
        ...
        if (pba->has_drad == _TRUE_)
          ppt->has_source_delta_drad = _TRUE_;
        if (pba->has_ncdm == _TRUE_)
      ...
      if (ppt->has_velocity_transfers == _TRUE_) {
        ...
        if ((pba->has_cdm == _TRUE_) && (ppt->gauge != synchronous || pba->has_drad == _TRUE_)){
        ...
        if (pba->has_drad == _TRUE_)
          ppt->has_source_theta_drad = _TRUE_;
        if (pba->has_ncdm == _TRUE_)
      ...
      class_define_index(ppt->index_tp_delta_drad,   ppt->has_source_delta_drad,  index_type,1);
      ...
      class_define_index(ppt->index_tp_theta_drad,   ppt->has_source_theta_drad,  index_type,1);
...
int perturb_workspace_init(
  ...
  if (_scalars_) {
    ...
    //if (pba->has_drad == _TRUE_) ppw->max_l_max = MAX(ppw->max_l_max, ppr->l_max_drad);
  }
  if (_tensors_) {
...
int perturb_solve(
  ...
  for (index_interval=0; index_interval<interval_number; index_interval++) {
    ...
    /*
    printf("pt_size %d index interval %d\n",ppw->pv->pt_size,index_interval);
    int i;
    for(i =0; i<26; i++)
      printf("ppw->y %lf usedinsources %d\n", ppw->pv->y[i],ppw->pv->used_in_sources[i]);
    */
...
int perturb_prepare_output(struct background * pba,
  ...
  if (ppt->k_output_values_num > 0) {
    ...
    if (ppt->has_scalars == _TRUE_){
      ...
      /* Decay radiation */
      class_store_columntitle(ppt->scalar_titles, "delta_drad", pba->has_drad);
      class_store_columntitle(ppt->scalar_titles, "theta_drad", pba->has_drad);
...
int perturb_find_approximation_switches(
  ...
  /** - if there are switches, consider approximations one after each
    ...
    for (index_switch=1; index_switch<interval_number; index_switch++) {
	  ...
	  //fprintf(stdout,"Mode k=%e: will switch on radiation streaming approximation at t \au=%e\n",ppt->k[index_md][index_k],interval_limit[index_switch]);
...
int perturb_vector_init(
  ...
  /*
    if (pba->has_drad == _TRUE_) {
      class_test(ppr->l_max_drad < 4,
                 ppt->error_message,
                 "ppr->l_max_drad should be at least 4, i.e. we must integrate at least over neutrino/relic density, velocity, shear, third and fourth momentum");
		 }*/
    ...
    class_define_index(ppv->index_pt_theta_cdm,pba->has_cdm && (ppt->gauge == newtonian || pba->has_drad),index_pt,1); /* cdm velocity */
    ...
    /* drad */
    if (pba->has_drad == _TRUE_){
      //ppv->l_max_drad = ppr->l_max_drad;
    class_define_index(ppv->index_pt_delta_drad,pba->has_drad,index_pt,1); /* dcdm density */
    class_define_index(ppv->index_pt_theta_drad,pba->has_drad,index_pt,1); /* dcdm velocity */
    }
  ...
  if (pa_old == NULL) {
    ...
    if (_tensors_) {
      //fprintf(stdout,"Initializing vector tensors");
      class_test(ppw->approx[ppw->index_ap_tca] == (int)tca_off,
                 ppt->error_message,
  ...
  /** - case of switching approximation while a wavenumber is being integrated */
    ...
    if (_scalars_) {
      ...
      if (pba->has_cdm == _TRUE_) {
        ...
        if (ppt->gauge == newtonian || pba->has_drad == _TRUE_) {
	...
	ppv->y[ppv->index_pt_delta_drad] =
          ppw->pv->y[ppw->pv->index_pt_delta_drad];

        ppv->y[ppv->index_pt_theta_drad] =
          ppw->pv->y[ppw->pv->index_pt_theta_drad];
...
int perturb_initial_conditions(struct precision * ppr,
  ...
  double delta_dr=0;
  double delta_drad=0., theta_drad=0.;
  double q,epsilon,k2;
  ...
  if (_scalars_) {
    ...
    if (pba->has_drad == _TRUE_) {
      rho_r += ppw->pvecback[pba->index_bg_rho_drad];
      //rho_nu += ppw->pvecback[pba->index_bg_rho_drad];
    }
    ...
    */
    ...
    if ((ppt->has_ad == _TRUE_) && (index_ic == ppt->index_ic_ad)) {
      ...
      /* all relativistic relics: drad*/

      if ((pba->has_drad == _TRUE_)) {
        delta_drad = ppw->pv->y[ppw->pv->index_pt_delta_g]; /* density of drad */

        theta_drad = - k*ktau_three/36./(4.*fracnu+15.) * (4.*fracnu+11.+12.*s2_squared-3.*(8.*fracnu*fracnu+50.*fracnu+275.)/20./(2.*fracnu+15.)*tau*om) * ppr->curvature_ini * s2_squared; /* velocity of ultra-relativistic neutrinos/relics */ //TBC   
    ...
    if (ppt->gauge == newtonian) {
      ...
      */
      ...
      if ((pba->has_ur == _TRUE_) || (pba->has_ncdm == _TRUE_) || (pba->has_dr == _TRUE_)) { //HERE
    ...
    if (pba->has_drad == _TRUE_) {
      ...
      ppw->pv->y[ppw->pv->index_pt_delta_drad] = delta_drad;

      ppw->pv->y[ppw->pv->index_pt_theta_drad] = theta_drad;
...
int perturb_total_stress_energy(
  ...
  if (_scalars_) {
    ...
    if (pba->has_drad == _TRUE_) {
      ppw->delta_rho += ppw->pvecback[pba->index_bg_rho_drad]*delta_drad;
      ppw->rho_plus_p_theta += 4./3.*ppw->pvecback[pba->index_bg_rho_drad]*theta_drad;
      //ppw->rho_plus_p_shear += 4./3.*ppw->pvecback[pba->index_bg_rho_drad]*shear_drad;
      ppw->delta_p += 1./3.*ppw->pvecback[pba->index_bg_rho_drad]*delta_drad;
    }
...
int perturb_sources(
  ...
  if (_scalars_) {
    ...
    /* delta_drad */
    if (ppt->has_source_delta_drad == _TRUE_) {
      _set_source_(ppt->index_tp_delta_drad) = y[ppw->pv->index_pt_delta_drad];
    ...
    /* theta_drad */
    if (ppt->has_source_theta_drad == _TRUE_) {
      _set_source_(ppt->index_tp_theta_drad) = y[ppw->pv->index_pt_theta_drad];
...
int perturb_print_variables(double tau,
  ...
  if (_scalars_) {
    ...
    /* Drad */
    class_store_double(dataptr, delta_drad, pba->has_drad, storeidx);
    class_store_double(dataptr, theta_drad, pba->has_drad, storeidx);
...
int perturb_derivs(double tau,
  ...
  if (_scalars_) {
    ...
    if (pba->has_drad == _TRUE_){
      if (ppt->gauge == newtonian){
        dy[pv->index_pt_delta_drad] = -4*y[pv->index_pt_theta_drad]/3-metric_continuity*4/3;
        dy[pv->index_pt_theta_drad] = k2*y[pv->index_pt_delta_drad]/4+metric_euler+3*pvecback[pba->index_bg_a]*(pvecthermo[pth->index_th_Tb]/pba->T_cmb)*(pvecthermo[pth->index_th_Tb]/pba->T_cmb)*pba->Gamma_drad*(y[pv->index_pt_theta_cdm]-y[pv->index_pt_theta_drad])*ppw->pvecback[pba->index_bg_rho_cdm]/ppw->pvecback[pba->index_bg_rho_drad]/4;

      }
      else{ //synchronous gauge
	dy[pv->index_pt_delta_drad] = -4*y[pv->index_pt_theta_drad]/3-metric_continuity*4/3;
	dy[pv->index_pt_theta_drad] = k2*y[pv->index_pt_delta_drad]/4+3*pvecback[pba->index_bg_a]*(pvecthermo[pth->index_th_Tb]/pba->T_cmb)*(pvecthermo[pth->index_th_Tb]/pba->T_cmb)*pba->Gamma_drad*(y[pv->index_pt_theta_cdm]-y[pv->index_pt_theta_drad])*ppw->pvecback[pba->index_bg_rho_cdm]/ppw->pvecback[pba->index_bg_rho_drad]/4;
	//printf("theta %f %f %f %f %f %f %f %f",k2*y[pv->index_pt_delta_drad]/4,3*pvecback[pba->index_bg_a],(pvecthermo[pth->index_th_Tb]/pba->T_cmb), pba->Gamma_drad, (y[pv->index_pt_theta_cdm]-y[pv->index_pt_theta_drad]), ppw->pvecback[pba->index_bg_rho_cdm], ppw->pvecback[pba->index_bg_rho_drad]/4, dy[pv->index_pt_theta_drad]);
      }
    }
	...
	if(pba->has_drad == _TRUE_)
          dy[pv->index_pt_theta_cdm] =  -a_prime_over_a*y[pv->index_pt_theta_cdm] + metric_euler + pvecback[pba->index_bg_a] * (pvecthermo[pth->index_th_Tb]/pba->T_cmb) * (pvecthermo[pth->index_th_Tb]/pba->T_cmb) * pba->Gamma_drad * (y[pv->index_pt_theta_drad]-y[pv->index_pt_theta_cdm]);
	else
	...
	if(pba->has_drad == _TRUE_){
	  dy[pv->index_pt_delta_cdm] -= -y[pv->index_pt_theta_cdm]; /* cdm density */
	  dy[pv->index_pt_theta_cdm] = -a_prime_over_a*y[pv->index_pt_theta_cdm] + pvecback[pba->index_bg_a]*(pvecthermo[pth->index_th_Tb]/pba->T_cmb) * (pvecthermo[pth->index_th_Tb]/pba->T_cmb) * pba->Gamma_drad*(-y[pv->index_pt_theta_cdm]+y[pv->index_pt_theta_drad]);
	}
  ...
  //6th is being set to infinity FIX THIS!!!!
  /*
  printf("%d %d ",pv->index_pt_delta_drad,pv->index_pt_theta_drad);
  int i;
  for(i=0;i<27;i++)
  printf("i %d %f ",i,dy[i]);*/
  return _SUCCESS_;
