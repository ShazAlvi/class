...
int background_functions(
  ...
  if (pba->has_drad == _TRUE_) {
    /* Pass value of rho_drad to output */
    //pvecback[pba->index_bg_rho_drad] = pvecback_B[pba->index_bi_rho_drad];
    //printf("computing drad density %f",pba->Omega0_drad);
    pvecback[pba->index_bg_rho_drad] = pba->Omega0_drad * pow(pba->H0,2) / pow(a_rel,4);   
    rho_tot += pvecback[pba->index_bg_rho_drad];
    p_tot += (1./3.)*pvecback[pba->index_bg_rho_drad];
    rho_r += pvecback[pba->index_bg_rho_drad];
  }
...
int background_indices(
  ...
  /* - index for ultra-relativistic neutrinos/species */
  class_define_index(pba->index_bg_rho_drad,pba->has_drad,index_bg,1);
  ...
  /* -> energy density in DRAD */
  class_define_index(pba->index_bi_rho_drad,pba->has_drad,index_bi,1);
...
int background_solve(
  ...
  //comment out
  if (pba->has_drad == _TRUE_){
    pba->Omega0_drad = pvecback_integration[pba->index_bi_rho_drad]/pba->H0/pba->H0;
    }*/
...
int background_initial_conditions(
  ...
  */
  ...
  /*
  if (pba->has_drad == _TRUE_){
    f = 1./3.*pow(a/pba->a_today,6)*pvecback_integration[pba->index_bg_rho_cdm]*pba->Gamma_drad/pow(pba->H0,3)/sqrt(Omega_rad);
      pvecback_integration[pba->index_bg_rho_drad] = f*pba->H0*pba->H0/pow(a/pba->a_today,4);
      }*/
...
int background_output_titles(struct background * pba,
  ...
  class_store_columntitle(titles,"(.)rho_drad",pba->has_drad);
...
int background_output_data(
  ...
  for (index_tau=0; index_tau<pba->bt_size; index_tau++){
    ...
    class_store_double(dataptr,pvecback[pba->index_bg_rho_drad],pba->has_drad,storeidx);
...
int background_derivs(
  ...
  if ((pba->has_drad == _TRUE_)){
    ...
    dy[pba->index_bi_rho_drad] = -4.*y[pba->index_bi_a]*pvecback[pba->index_bg_H]*y[pba->index_bi_rho_drad]+
      y[pba->index_bi_a]*pba->Gamma_drad*y[pba->index_bg_rho_cdm];
  }*/
