...
int input_read_parameters(
  ...
  /*
  // Omega_0_drad 
  class_call(parser_read_double(pfc,"N_drad",&param1,&flag1,errmsg),
             errmsg,
             errmsg);
  pba->Omega0_drad = param1*7./8.*pow(4./11.,4./3.)*pba->Omega0_g;
  printf("omega0_drad %lf %lf\n",pba->Omega0_drad, param1);

  Omega_tot += pba->Omega0_drad;


  // Read Gamma in  Mpc-1
  class_read_double("Gamma_drad",pba->Gamma_drad);
  printf("read Gamma_drad %lf",pba->Gamma_drad);*/
  ...
  class_test((flag1 == _TRUE_) && (flag2 == _TRUE_),
             errmsg,
             "In input file, you can only enter one of N_eff (deprecated syntax) or N_ur (up-to-date syntax), since they both describe the same, i.e. the contribution ukltra-relativistic species to the effective neutrino number");
  if (flag2 == _TRUE_) {
    param1 = param2;
    flag1 = _TRUE_;
    flag2 = _FALSE_;
  ...
  //         errmsg,
  //           errmsg);
  class_read_double("N_drad",pba->N_drad);
  //OLD pba->Omega0_drad = param1*pow(4./11.,4./3.)*pba->Omega0_g;

  printf(" ndrad %f \n",pba->N_drad);
  pba->Omega0_drad = pba->N_drad*7./8.*pow(4./11.,4./3.)*pba->Omega0_g;
  printf("omega0_drad %lf %lf\n",pba->Omega0_drad, pba->N_drad);
  ...
  /* Read Gamma in  Mpc-1*/
  class_read_double("Gamma_drad",pba->Gamma_drad);
  printf("read Gamma_drad %e",pba->Gamma_drad);
  ...
  */
  ...
  //class_read_int("l_max_drad",ppr->l_max_drad);
...
int input_default_params(
  ...
  pba->K = 0.;
  pba->sgnK = 0;
  pba->Omega0_lambda = 1.-pba->Omega0_k-pba->Omega0_g-pba->Omega0_ur-pba->Omega0_b-pba->Omega0_cdm-pba->Omega0_ncdm_tot-pba->Omega0_dcdmdr-pba->Omega0_drad;
  pba->Omega0_fld = 0.;
  pba->a_today = 1.;
  pba->w0_fld=-1.;
  pba->wa_fld=0.;
...
int input_try_unknown_parameters(double * unknown_parameter,
  ...
  int i;
  double rho_dcdm_today, rho_dr_today, rho_drad_today;
