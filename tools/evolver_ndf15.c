...
int new_linearisation(struct jacobian *jac,double hinvGak,int neq,ErrorMsg error_message){
  ...
  if(jac->use_sparse==1){
    ...
    //printf("normal jacobian calculation \n");
    /* Normal calculation: */
    for(i=1;i<=neq;i++){
      for(j=1;j<=neq;j++){
	jac->LU[i][j] = - hinvGak * jac->dfdy[i][j];
	//printf("hinv dfdy %f %f ",hinvGak, jac->dfdy[i][j]);
  ...
  */
  ...
  if ((jac->use_sparse)&&(jac->repeated_pattern >= jac->trust_sparse)){
    ...
    for(j=1;j<=neq;j++){
      for(i=1;i<=neq;i++){
	nj_ws->ydel_Fdel[i][j] = y[i];
	//printf("3 njws %d %d %f ",i,j,y[i]);
      }
      nj_ws->ydel_Fdel[j][j] += nj_ws->del[j];
      //printf("4 njws %d %d %f ",i,j,nj_ws->del[j]);
    }
  }
  ...
  for(j=1;j<=colmax;j++){
    ...
    *nfe+=1;
    for(i=1;i<=neq;i++){
      ...
      //printf("5 njws %d %d %f ",i,j,nj_ws->ydel_Fdel[i][j]); //ffdel
    }
  ...
  if ((jac->use_sparse)&&(jac->repeated_pattern >= jac->trust_sparse)){
	...
	//printf("normal dfdy %d %d %f %f %f\n",i,j,fval[i],nj_ws->ydel_Fdel[i][j],nj_ws->del[j]);
	/*Find row maximums:*/
	if(fabs(Fdiff_new)>=Fdiff_absrm){
